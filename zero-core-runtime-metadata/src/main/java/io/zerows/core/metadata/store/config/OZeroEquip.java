...
public class OZeroEquip extends AbstractAmbiguity implements HEquip {
    ...
    public HSetting initialize() {
         ...
         *
         */
        final JsonObject configuration = Ut.Bnd.ioCombineJ(this.nameZero(null), this.bundle); // ZeroIo.read(null, true);
        ...
        keys.stream().filter(field -> !internal.contains(field)).forEach(field -> {
            ...
            // - vertx-key2.yml
            // - vertx-key3.yml
            final JsonObject fileData = Ut.Bnd.ioConfigureJ(this.nameZero(field), this.bundle); // ZeroIo.read(field, false);
        ...
        Arrays.stream(KPlugin.FILE_KEY).forEach(field -> {
            final JsonObject fileData = Ut.Bnd.ioCombineJ(this.nameZero(field), this.bundle); // ZeroIo.read(field, true);
