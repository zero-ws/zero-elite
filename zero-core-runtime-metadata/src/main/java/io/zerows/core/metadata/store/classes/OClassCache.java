 ...
 */
public class OClassCache extends AbstractAmbiguity implements OClassProcessor {
    ...
    private OClassCache(final Bundle bundle) {
        super(bundle);
        ...
        Ut.Log.metadata(this.getClass()).info(OClassCache.INFO_SCANNED,
     ...
     * @return OCache
     */
    public static OClassCache of(final Bundle bundle) {
        final String key = bundle.getSymbolicName() + VString.SLASH + bundle.getVersion().getQualifier();
        return CC_SCANNED.pick(() -> new OClassCache(bundle), key);
    }

    public static OClassCache of() {
        return CC_SCANNED.pick(() -> new OClassCache(null), KMeta.Component.DEFAULT_SCANNED);
    }
    ...
    @Override
    public OClassProcessor compile(final KMeta.Typed type, final Function<Set<Class<?>>, Set<Class<?>>> compiler) {
     ...
     */
    public static class OClassCacheInternal {
        ...
        public static OClassCacheInternal of() {
            return new OClassCacheInternal();
        }

        public OClassCacheInternal add(final Set<Class<?>> classes) {
            this.classSet.addAll(classes);
            return this;
        }

        public OClassCacheInternal addBy(final KMeta.Typed type, final Set<Class<?>> classes) {
         ...
         * @return this
         */
        public OClassCacheInternal compile(final KMeta.Typed type, final Function<Set<Class<?>>, Set<Class<?>>> compiler) {
            return this.addBy(type, compiler.apply(this.classSet));
        }
        ...
        public OClassCacheInternal remove(final KMeta.Typed type) {
            final Set<Class<?>> removed = this.classMap.remove(type);
            this.classSet.removeAll(removed);
            return this;
        }

        public OClassCacheInternal remove(final Class<?> clazz) {
            // 基础删除
        ...
        public OClassCacheInternal remove(final Set<Class<?>> classes) {
            // 基础删除
